...
mechanical expansion of that locale's own ``<intent>.intent`` padatious
template: ``(a|b|c)`` word-choice groups (including a trailing
``(x)?`` shorthand) are resolved to one alternative, ``[optional]``
...
is stubbed True, and the colour/brightness handlers' blocking
``ask_yesno``/``get_response`` prompts are short-circuited before any
``MiniCroft`` boots.

One ``MiniCroft`` is booted per locale (class-scoped, torn down after),
mirroring the other skills' multilang suites in this batch.
...
LANGS = [
    "en-US", "ca-ES", "da-DK", "de-DE", "el-GR", "es-ES", "eu-ES",
    "fr-FR", "gl-ES", "it-IT", "kab", "nl-NL", "pt-BR", "pt-PT",
    "ru-RU", "sv-SE",
]
